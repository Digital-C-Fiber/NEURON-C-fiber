...
#dimension: number of ion channels to be optimized
def update_particle(particle, global_best_position, inertia_weight, cognitive_weight, social_weight, dimension):
    r1, r2 = random.random(), random.random()
    #adjust weights to fit scale of each channel
    #cognitive_weights=np.array([cognitive_weight * channel_scales[i] for i in range(dimension)])
    cognitive_weights=np.array([cognitive_weight for i in range(dimension)])
    cognitive_component = np.array([cognitive_weights[i] * r1 * (particle.best_position[i] - particle.position[i]) for i in range(dimension)])
    #adjust weights to fit scale of each channel
    #social_weights=np.array([social_weight * channel_scales[i] for i in range(dimension)])
    social_weights=np.array([social_weight for i in range(dimension)])
    social_component = np.array([social_weights[i] * r2 * (global_best_position[i] - particle.position[i]) for i in range(dimension)])
    particle.velocity = inertia_weight * particle.velocity + cognitive_component + social_component
    #print("Velocity:"+str(particle.velocity))
    #particle.position += checkVelocity(particle.velocity, dimension)
...
def particle_swarm_optimization(dimension, num_particles, num_iterations, inertia_weight, cognitive_weight, social_weight, name, prot):
    #start timer
    ...
    with Pool() as pool:
        for _ in range(num_iterations):
            ...
            print(f"Simulation time Run and evaluate: {(toc2 - tic2)/60:0.4f} min")
            ...
            for particle in swarm:
                update_particle(particle, global_best_position, inertia_weight, cognitive_weight, social_weight, dimension)
