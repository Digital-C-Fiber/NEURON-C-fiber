...
def insertChannels(axon, condFactor, gPump=-0.0047891, gNav13=0.010664, gNav17=0.10664, gNav18=0.24271, gNav19=9.4779e-05, gKs=0.0069733, gKf=0.012756, gH=0.0025377, gKdr=0.018002, gKna=0.00042):
    axon.insert('ks')
    axon.insert('kf')
    axon.insert('h')
    axon.insert('nav1p3')                   #
    axon.insert('nav1p3_h')
    axon.insert('nattxs')
    axon.insert('nattxs_h')
    axon.insert('nav1p8')
    axon.insert('nav1p8_h')
    axon.insert('nav1p9')
    axon.insert('nakpump')
    ...
    for seg in axon:
        ...
        # seg.nav1p3.gbar = 0.010664#Nav1.3
        seg.nav1p3.gbar = gNav13*condFactor     #
        h.setpointer(                           # pointer
            seg.nav1p3_h._ref_h0,
            "h0_source",
            seg.nav1p3
        )
...
def setTemp(axon, tempCelsius):
    for seg in axon:
        ...
        seg.nav1p3.celsiusT = tempCelsius       #
        seg.nav1p3_h.celsiusT = tempCelsius
        seg.nattxs.celsiusT = tempCelsius
        seg.nattxs_h.celsiusT = tempCelsius
        seg.nav1p8.celsiusT = tempCelsius
        seg.nav1p8_h.celsiusT = tempCelsius
...
def balance(axon, Vrest):
    inaSum = -(axon.ina_nav1p3 + axon.ina_nattxs + axon.ina_nav1p9 +
               axon.ina_nav1p8 + axon.ina_h+axon.ina_nakpump)
    if (inaSum/(Vrest-axon.ena)) < 0:
        axon.pumpina_extrapump = inaSum
    else:
        axon.gnaleak_leak = inaSum/(Vrest-axon.ena)

    ikSum = -(axon.ik_ks+axon.ik_kf + axon.ik_h + axon.ik_kdr +
              axon.ik_nakpump + axon.ik_kna)  # Tigerholm
