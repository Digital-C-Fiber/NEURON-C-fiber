...
def setStimulationProtocol(axon, prot, previousStim):
    ...
    if prot == -1:
        ...
        extras = [2000, 1750, 1500, 1250, 1000, 750, 500, 250, 150,
                  100, 75, 50, 40, 30, 20, 10]  # distance to next regular puls
    ...
    elif prot == 22:                                    # prot16, baseline 0.5Hz
        # 20 baseline pulses at 0.5 Hz
        offset = 0
        for i in range(1, 21):
            vec.append(offset + i*2000)

        lastPulse = offset + 20*2000
        extras = [1750, 1500, 1250, 1000, 750, 500, 250, 150,
    ...
    elif prot == 23:                                    # prot16, baseline 1Hz
        # 20 baseline pulses at 1 Hz
        offset = 0
        for i in range(1, 21):
            vec.append(offset + i*1000)
        lastPulse = offset + 20*1000
        extras = [750, 500, 250, 150, 100, 75, 50, 40, 30, 20, 10]
        numReg = 5
        for e in extras:
            # one extra pulse
    ...
    elif prot == 24:                                    # prot16, baseline 0.6Hz
        # 20 baseline pulses at 0.6 Hz
        interval = 1000.0 / 0.6
        offset = 0
        for i in range(1, 21):
            vec.append(offset + i*interval)
        lastPulse = offset + 20*interval
        extras = [1500, 1250, 1000, 750, 500,
                  250, 150, 100, 75, 50, 40, 30, 20, 10]
        numReg = 5
        for e in extras:
            # one extra pulse
            vec.append(lastPulse + interval - e)
            # 5 regular pulses
            for i in range(1, numReg+1):
                vec.append(lastPulse + i*interval)
            lastPulse = lastPulse + numReg*interval
        delay = lastPulse + interval

    elif prot == 25:                                    # prot16, baseline 0.4Hz
        # 20 baseline pulses at 0.4 Hz
        interval = 1000.0 / 0.4
        offset = 0
        for i in range(1, 21):
            vec.append(offset + i*interval)
        lastPulse = offset + 20*interval
        extras = [2000, 1750, 1500, 1250, 1000, 750, 500,
                  250, 150, 100, 75, 50, 40, 30, 20, 10]
        numReg = 5
        for e in extras:
            # one extra pulse
...
def getStimProt():
    vec = []
    delay = 0
    # waittime between the regular pulses (underlying frequency)
    RegPulse = 4000
