...
#dimension: number of ion channels to be optimized
def update_particle(particle, randomParticle, inertia_weight, cognitive_weight, social_weight, rand_velocity_weight, dimension):
    r1, r2 = random.random(), random.random()
    cognitive_component = np.array([(particle.best_position[i] - particle.position[i]) for i in range(dimension)])
    
    social_component = np.array([(randomParticle.best_position[i] - particle.position[i]) for i in range(dimension)])
    ...
    particle.velocity = inertia_weight * particle.velocity + cognitive_weight * r1 *cognitive_component + social_weight * r2 * social_component + rand_velocity_weight * rand_velocity
...
def particle_swarm_optimization(dimension, num_particles, num_iterations, inertia_weight, cognitive_weight, social_weight, rand_velocity_weight, name, prot):
    #start timer
    ...
    with Pool() as pool:
        for _ in range(num_iterations):
            ...
            for particle in swarm:
                ...
                update_particle(particle, swarm[r1], inertia_weight, cognitive_weight, social_weight, rand_velocity_weight, dimension)
